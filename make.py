...
TF_BACKEND_LOCATION = sane_utils.get_var_for_target('PROJECT_ID', TARGETS[0], False, default="EU")
...
def create_tf_vars(
        ...
        content=lambda: {
            ...
            "targets_with_cluster": {e["name"]: e for e in TARGET_DICTS if bool(int(sane_utils.get_var_for_target(
                "USE_CLUSTER", e["name"])))},
            "targets_with_cloudrun": {e["name"]: e for e in TARGET_DICTS if bool(int(sane_utils.get_var_for_target(
                "USE_CLOUDRUN", e["name"])))},
